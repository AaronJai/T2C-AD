...
# ── Unambiguous Q-001 .. 040 ──────────────────────────────────────────────────
ITEMS["Q-001"] = ("MATCH (p:Person)-[:SUSPECTED_OF]->(:Incident {crime_type:'homicide'}) RETURN p.name", [])
ITEMS["Q-002"] = ("MATCH (p:Person)-[:INVESTIGATES]->(:Incident {crime_type:'homicide'}) RETURN p.name", [])
ITEMS["Q-003"] = ("MATCH (p:Person)-[:VICTIM_OF]->(:Incident {crime_type:'homicide'}) RETURN p.name", [])
ITEMS["Q-004"] = ("MATCH (:Case {case_id:'CASE-001'})-[:CONTAINS]->(i:Incident) RETURN i.incident_id", [])
ITEMS["Q-005"] = ("MATCH (c:Case)-[:CONTAINS]->(:Incident {crime_type:'homicide'}) RETURN c.case_name", [])
ITEMS["Q-006"] = ("MATCH (:Person {name:'Daniel Kim'})-[:USES_PHONE {active:true}]->(ph:Phone) RETURN ph.phone_number", [])
ITEMS["Q-007"] = ("MATCH (:Person {name:'James Whitfield'})-[:OWNS {active:true}]->(v:Vehicle) RETURN v.vehicle_id", [])
ITEMS["Q-008"] = ("MATCH (p:Person)-[:LIVES_AT {active:true}]->(:Location {address:'55 Beaufort Street'}) RETURN p.name", [])
ITEMS["Q-009"] = ("MATCH (:Person {name:'Anton Maric'})-[:ASSOCIATED_WITH {active:true}]-(o:Person) RETURN o.name", [])
ITEMS["Q-010"] = ("MATCH (:Phone {phone_number:'0412 345 678'})-[:CALLED]->(b:Phone) RETURN DISTINCT b.phone_number", [])
ITEMS["Q-011"] = ("MATCH (i:Incident)-[:OCCURRED_AT]->(:Location {suburb:'Northbridge'}) RETURN i.incident_id", [])
ITEMS["Q-012"] = ("MATCH (p:Person)-[:SUSPECTED_OF]->(i:Incident)-[:OCCURRED_AT]->(l:Location) WHERE l.address CONTAINS 'William Street' RETURN p.name", [])
ITEMS["Q-013"] = ("MATCH (i:Incident {crime_type:'burglary'})-[:OCCURRED_AT]->(l:Location {suburb:'Fremantle'}) RETURN l.address", [])
ITEMS["Q-014"] = ("MATCH (p:Person)-[:WITNESSED]->(i:Incident {crime_type:'burglary'})-[:OCCURRED_AT]->(:Location {suburb:'Fremantle'}) RETURN p.name", [])
ITEMS["Q-015"] = ("MATCH (p:Person)-[:INVESTIGATES]->(i:Incident {crime_type:'assault'})-[:OCCURRED_AT]->(:Location {suburb:'Scarborough'}) RETURN p.name", [])
ITEMS["Q-016"] = ("MATCH (:Incident {crime_type:'homicide'})-[:OCCURRED_AT]->(l:Location) RETURN l.address", [])
ITEMS["Q-017"] = ("MATCH (:Person {name:'Ricky Santos'})-[:OWNS {active:false}]->(v:Vehicle) RETURN v.vehicle_id", [])
ITEMS["Q-018"] = ("MATCH (p:Person)-[:USES_PHONE {active:true}]->(:Phone {phone_number:'0423 987 654'}) RETURN p.name", [])
ITEMS["Q-019"] = ("MATCH (:Phone {phone_number:'0478 444 555'})-[:CALLED]->(b:Phone) RETURN DISTINCT b.phone_number", [])
ITEMS["Q-020"] = ("MATCH (p:Person)-[:LIVES_AT {active:true}]->(:Location {address:'35 Walcott Street'}) RETURN p.name", [])
ITEMS["Q-021"] = ("MATCH (:Case {case_id:'CASE-001'})-[:CONTAINS]->(i:Incident)<-[:SUSPECTED_OF]-(p:Person) RETURN DISTINCT p.name", [])
ITEMS["Q-022"] = ("MATCH (:Case {case_id:'CASE-004'})-[:CONTAINS]->(i:Incident)<-[:INVESTIGATES]-(p:Person) RETURN DISTINCT p.name", [])
ITEMS["Q-023"] = ("MATCH (:Case {case_id:'CASE-002'})-[:CONTAINS]->(i:Incident)-[:OCCURRED_AT]->(l:Location) RETURN DISTINCT l.suburb", [])
ITEMS["Q-024"] = ("MATCH (:Person {name:'James Whitfield'})-[:USES_PHONE {active:true}]->(:Phone)-[:CALLED]->(b:Phone) RETURN DISTINCT b.phone_number", [])
ITEMS["Q-025"] = ("MATCH (:Person {name:'Anton Maric'})-[:ASSOCIATED_WITH {active:true}]-(a:Person)-[:SUSPECTED_OF]->(:Incident) RETURN DISTINCT a.name", [])
ITEMS["Q-026"] = ("MATCH (p:Person)-[:SUSPECTED_OF]->(i:Incident)-[:OCCURRED_AT]->(l:Location), (p)-[:LIVES_AT {active:true}]->(home:Location) WHERE l.address CONTAINS 'William Street' RETURN DISTINCT home.address", [])
ITEMS["Q-027"] = ("MATCH (p:Person)-[:SUSPECTED_OF]->(:Incident {crime_type:'homicide'}), (p)-[:OWNS {active:true}]->(v:Vehicle) RETURN v.vehicle_id", [])
ITEMS["Q-028"] = ("MATCH (p:Person)-[:WITNESSED]->(i:Incident)-[:OCCURRED_AT]->(l:Location), (p)-[:USES_PHONE {active:true}]->(ph:Phone) WHERE l.address CONTAINS 'William Street' RETURN ph.phone_number", [])
ITEMS["Q-029"] = ("MATCH (p:Person)-[:INVESTIGATES]->(i:Incident {crime_type:'fraud'})-[:OCCURRED_AT]->(l:Location) WHERE l.address CONTAINS 'St Georges Terrace' RETURN p.name", [])
ITEMS["Q-030"] = ("MATCH (p:Person)-[:LIVES_AT {active:true}]->(l:Location {suburb:'Cannington'}) WHERE l.address CONTAINS 'Albany Highway' RETURN p.name", [])
ITEMS["Q-031"] = ("MATCH (c:Case)-[:CONTAINS]->(i:Incident {crime_type:'robbery'})-[:OCCURRED_AT]->(:Location {suburb:'Midland'}) RETURN c.case_name", [])
ITEMS["Q-032"] = ("MATCH (p:Person)-[:VICTIM_OF]->(i:Incident {crime_type:'burglary'})-[:OCCURRED_AT]->(:Location {suburb:'Fremantle'}) RETURN p.name", [])
ITEMS["Q-033"] = ("MATCH (a:Phone)-[:CALLED]->(:Phone)<-[:USES_PHONE {active:true}]-(:Person {name:'Ricky Santos'}) RETURN DISTINCT a.phone_number", [])
ITEMS["Q-034"] = ("MATCH (:Person {name:'Amir Hassan'})-[:OWNS {active:true}]->(v:Vehicle) RETURN v.make, v.model", [])
ITEMS["Q-035"] = ("MATCH (p:Person)-[:WITNESSED]->(i:Incident)-[:OCCURRED_AT]->(l:Location) WHERE l.address CONTAINS 'Lake Street' RETURN p.name", [])
ITEMS["Q-036"] = ("MATCH (:Case {case_id:'CASE-003'})-[:CONTAINS]->(i:Incident)<-[:INVESTIGATES]-(p:Person) RETURN DISTINCT p.name", [])
ITEMS["Q-037"] = ("MATCH (:Incident {crime_type:'assault'})-[:OCCURRED_AT]->(l:Location {suburb:'Northbridge'}) RETURN l.address", [])
ITEMS["Q-038"] = ("MATCH (p:Person)-[:OWNS {active:true}]->(:Vehicle {make:'Holden', model:'Commodore'}) RETURN p.name", [])
ITEMS["Q-039"] = ("MATCH (i:Incident)-[:OCCURRED_AT]->(:Location {suburb:'Perth'}) RETURN i.incident_id", [])
ITEMS["Q-040"] = ("MATCH (p:Person)-[:SUSPECTED_OF]->(i:Incident)-[:OCCURRED_AT]->(l:Location) WHERE l.address CONTAINS 'Hay Street' RETURN p.name", [])

# ── Schema ambiguity Q-041 .. 060 ──────────────────────────────────────────────
ITEMS["Q-041"] = schema_item("INC-001", [S("INC-001"), W("INC-001"), I("INC-001")])
ITEMS["Q-042"] = schema_item("INC-006", [S("INC-006"), W("INC-006"), I("INC-006")])
ITEMS["Q-043"] = schema_item("INC-005", [S("INC-005"), W("INC-005"), I("INC-005")])
ITEMS["Q-044"] = schema_item("INC-009", [S1("INC-009"), W1("INC-009"), I1("INC-009")])
ITEMS["Q-045"] = schema_item("INC-007", [S("INC-007"), W("INC-007"), I("INC-007")])
ITEMS["Q-046"] = schema_item("INC-008", [S1("INC-008"), W1("INC-008"), I("INC-008")])
ITEMS["Q-047"] = schema_item("INC-004", [S1("INC-004"), W1("INC-004"), I1("INC-004")])
ITEMS["Q-048"] = schema_item("INC-006", [S("INC-006"), I("INC-006"), V("INC-006")])
ITEMS["Q-049"] = schema_item("INC-012", [S("INC-012"), W1("INC-012"), I("INC-012")])
ITEMS["Q-050"] = schema_item("INC-003", [S1("INC-003"), W("INC-003"), I("INC-003")])
ITEMS["Q-051"] = schema_item("INC-002", [S("INC-002"), W1("INC-002"), I("INC-002")])
ITEMS["Q-052"] = schema_item("INC-010", [S1("INC-010"), W1("INC-010"), I1("INC-010")])
ITEMS["Q-053"] = schema_item("INC-011", [S1("INC-011"), W1("INC-011"), I1("INC-011")])
ITEMS["Q-054"] = schema_item("INC-001", [S("INC-001"), V("INC-001"), W("INC-001")])
ITEMS["Q-055"] = schema_item("INC-005", [S("INC-005"), I("INC-005")])
ITEMS["Q-056"] = schema_item("INC-006", [S("INC-006"), W("INC-006"), I("INC-006")])
ITEMS["Q-057"] = schema_item("INC-008", [S1("INC-008"), V("INC-008"), I("INC-008")])
ITEMS["Q-058"] = schema_item("INC-006", [S("INC-006"), W("INC-006"), V("INC-006")])
ITEMS["Q-059"] = (
    "MATCH (:Case {case_id:'CASE-003'})-[:CONTAINS]->(i:Incident)<-[r]-(p:Person) RETURN DISTINCT p.name",
    [("Only suspects", "MATCH (:Case {case_id:'CASE-003'})-[:CONTAINS]->(i:Incident)<-[:SUSPECTED_OF]-(p:Person) RETURN DISTINCT p.name"),
     ("Only witnesses", "MATCH (:Case {case_id:'CASE-003'})-[:CONTAINS]->(i:Incident)<-[:WITNESSED]-(p:Person) RETURN DISTINCT p.name"),
     ("Only investigating officers", "MATCH (:Case {case_id:'CASE-003'})-[:CONTAINS]->(i:Incident)<-[:INVESTIGATES]-(p:Person) RETURN DISTINCT p.name")],
)
ITEMS["Q-060"] = schema_item("INC-004", [S1("INC-004"), W1("INC-004"), I1("INC-004")])

# ── Entity ambiguity Q-061 .. 080 ──────────────────────────────────────────────
ITEMS["Q-061"] = (
...
ITEMS["Q-066"] = (
    ...
    [("Sophie Bennett (PER-024)", "MATCH (:Person {name:'Sophie Bennett'})-[r]->(i:Incident) RETURN DISTINCT i.incident_id"),
     ("Sarah Bennett (PER-025)", "MATCH (:Person {name:'Sarah Bennett'})-[r]->(i:Incident) RETURN DISTINCT i.incident_id")],
)
ITEMS["Q-067"] = (
    "MATCH (p:Person)-[:LIVES_AT {active:true}]->(l:Location) WHERE p.name STARTS WITH 'James ' RETURN l.address",
    [("James Kowalski (PER-004)", "MATCH (:Person {name:'James Kowalski'})-[:LIVES_AT {active:true}]->(l:Location) RETURN l.address"),
     ("James Whitfield (PER-007)", "MATCH (:Person {name:'James Whitfield'})-[:LIVES_AT {active:true}]->(l:Location) RETURN l.address"),
     ("James Smith (PER-016)", "MATCH (:Person {name:'James Smith'})-[:LIVES_AT {active:true}]->(l:Location) RETURN l.address")],
)
ITEMS["Q-068"] = (
    "MATCH (p:Person)-[:LIVES_AT {active:true}]->(l:Location) WHERE p.name STARTS WITH 'Sarah ' RETURN l.address",
    [("Sarah Nguyen (PER-019)", "MATCH (:Person {name:'Sarah Nguyen'})-[:LIVES_AT {active:true}]->(l:Location) RETURN l.address"),
     ("Sarah Bennett (PER-025)", "MATCH (:Person {name:'Sarah Bennett'})-[:LIVES_AT {active:true}]->(l:Location) RETURN l.address")],
)
ITEMS["Q-069"] = (
    "MATCH (p:Person)-[r]->(i:Incident) WHERE p.name = 'David Chen' OR p.alias = 'David' RETURN DISTINCT i.incident_id",
    [("David Chen (PER-001, given name)", "MATCH (:Person {name:'David Chen'})-[r]->(i:Incident) RETURN DISTINCT i.incident_id"),
     ("Wei Chen (PER-012, alias 'David')", "MATCH (p:Person {alias:'David'})-[r]->(i:Incident) RETURN DISTINCT i.incident_id")],
)
ITEMS["Q-070"] = (
    "MATCH (i:Incident)-[:OCCURRED_AT]->(l:Location) WHERE l.address CONTAINS 'Albany Highway' RETURN i.incident_id",
    [("Albany Highway, Cannington — homicide (INC-006)", "MATCH (i:Incident)-[:OCCURRED_AT]->(l:Location {suburb:'Cannington'}) WHERE l.address CONTAINS 'Albany Highway' RETURN i.incident_id"),
     ("Albany Highway, Victoria Park — burglary (INC-011)", "MATCH (i:Incident)-[:OCCURRED_AT]->(l:Location {suburb:'Victoria Park'}) WHERE l.address CONTAINS 'Albany Highway' RETURN i.incident_id")],
)
ITEMS["Q-071"] = (
    "MATCH (p:Person)-[:LIVES_AT {active:true}]->(l:Location) WHERE l.address CONTAINS 'Albany Highway' RETURN p.name",
    [("Residents of Albany Highway, Cannington (LOC-009)", "MATCH (p:Person)-[:LIVES_AT {active:true}]->(l:Location {suburb:'Cannington'}) WHERE l.address CONTAINS 'Albany Highway' RETURN p.name"),
     ("Residents of Albany Highway, Victoria Park (LOC-013)", "MATCH (p:Person)-[:LIVES_AT {active:true}]->(l:Location {suburb:'Victoria Park'}) WHERE l.address CONTAINS 'Albany Highway' RETURN p.name")],
)
ITEMS["Q-072"] = (
    "MATCH (p:Person)-[r]->(i:Incident) WHERE p.name ENDS WITH ' Tran' RETURN DISTINCT i.incident_id",
    [("Megan Tran (PER-002, officer)", "MATCH (:Person {name:'Megan Tran'})-[r]->(i:Incident) RETURN DISTINCT i.incident_id"),
     ("Kevin Tran (PER-031, witness)", "MATCH (:Person {name:'Kevin Tran'})-[r]->(i:Incident) RETURN DISTINCT i.incident_id")],
)
ITEMS["Q-073"] = (
    "MATCH (p:Person)-[:LIVES_AT {active:true}]->(l:Location) WHERE p.name ENDS WITH ' Nguyen' RETURN l.address",
    [("Michael Nguyen (PER-015)", "MATCH (:Person {name:'Michael Nguyen'})-[:LIVES_AT {active:true}]->(l:Location) RETURN l.address"),
     ("Sarah Nguyen (PER-019)", "MATCH (:Person {name:'Sarah Nguyen'})-[:LIVES_AT {active:true}]->(l:Location) RETURN l.address")],
)
ITEMS["Q-074"] = (
    "MATCH (p:Person)-[r]->(i:Incident) WHERE p.name ENDS WITH ' Kim' RETURN DISTINCT i.incident_id",
    [("Daniel Kim (PER-009, suspect/witness)", "MATCH (:Person {name:'Daniel Kim'})-[r]->(i:Incident) RETURN DISTINCT i.incident_id"),
     ("Rachel Kim (PER-006, officer)", "MATCH (:Person {name:'Rachel Kim'})-[r]->(i:Incident) RETURN DISTINCT i.incident_id")],
)
ITEMS["Q-075"] = (
    "MATCH (p:Person)-[r]->(i:Incident) WHERE p.name STARTS WITH 'Sarah ' RETURN DISTINCT i.incident_id",
    [("Sarah Nguyen (PER-019)", "MATCH (:Person {name:'Sarah Nguyen'})-[r]->(i:Incident) RETURN DISTINCT i.incident_id"),
     ("Sarah Bennett (PER-025)", "MATCH (:Person {name:'Sarah Bennett'})-[r]->(i:Incident) RETURN DISTINCT i.incident_id")],
)
ITEMS["Q-076"] = (
    "MATCH (i:Incident {crime_type:'robbery'}) RETURN i.incident_id",
    [("The William Street robbery (INC-001)", "MATCH (i:Incident {crime_type:'robbery'})-[:OCCURRED_AT]->(:Location {suburb:'Northbridge'}) RETURN i.incident_id"),
     ("The Midland robbery (INC-008)", "MATCH (i:Incident {crime_type:'robbery'})-[:OCCURRED_AT]->(:Location {suburb:'Midland'}) RETURN i.incident_id")],
)
ITEMS["Q-077"] = (
    "MATCH (p:Person)-[:SUSPECTED_OF]->(i:Incident {crime_type:'robbery'}) RETURN DISTINCT p.name",
    [("Suspects of the William Street robbery (INC-001)", "MATCH (p:Person)-[:SUSPECTED_OF]->(i:Incident {crime_type:'robbery'})-[:OCCURRED_AT]->(:Location {suburb:'Northbridge'}) RETURN p.name"),
     ("Suspects of the Midland robbery (INC-008)", "MATCH (p:Person)-[:SUSPECTED_OF]->(i:Incident {crime_type:'robbery'})-[:OCCURRED_AT]->(:Location {suburb:'Midland'}) RETURN p.name")],
)
ITEMS["Q-078"] = (
    "MATCH (p:Person)-[:WITNESSED]->(i:Incident {crime_type:'assault'}) RETURN DISTINCT p.name",
    [("Witnesses of the Northbridge assault (INC-003)", "MATCH (p:Person)-[:WITNESSED]->(i:Incident {crime_type:'assault'})-[:OCCURRED_AT]->(:Location {suburb:'Northbridge'}) RETURN p.name"),
     ("Witnesses of the Scarborough assault (INC-009)", "MATCH (p:Person)-[:WITNESSED]->(i:Incident {crime_type:'assault'})-[:OCCURRED_AT]->(:Location {suburb:'Scarborough'}) RETURN p.name")],
)
ITEMS["Q-079"] = (
    "MATCH (i:Incident {crime_type:'fraud'})-[:OCCURRED_AT]->(l:Location) RETURN l.address",
    [("The St Georges Terrace fraud (INC-005)", "MATCH (i:Incident {crime_type:'fraud'})-[:OCCURRED_AT]->(l:Location) WHERE l.address CONTAINS 'St Georges Terrace' RETURN l.address"),
     ("The Hay Street fraud (INC-012)", "MATCH (i:Incident {crime_type:'fraud'})-[:OCCURRED_AT]->(l:Location) WHERE l.address CONTAINS 'Hay Street' RETURN l.address")],
)
ITEMS["Q-080"] = (
    "MATCH (p:Person)-[:INVESTIGATES]->(i:Incident {crime_type:'drug_offence'}) RETURN DISTINCT p.name",
    [("Investigators of the Midland drug offence (INC-002)", "MATCH (p:Person)-[:INVESTIGATES]->(i:Incident {crime_type:'drug_offence'})-[:OCCURRED_AT]->(:Location {suburb:'Midland'}) RETURN p.name"),
     ("Investigators of the Lake Street drug offence (INC-007)", "MATCH (p:Person)-[:INVESTIGATES]->(i:Incident)-[:OCCURRED_AT]->(l:Location) WHERE l.address CONTAINS 'Lake Street' RETURN p.name")],
)

# ── Intent ambiguity Q-081 .. 100 ──────────────────────────────────────────────
ITEMS["Q-081"] = (
    "MATCH (p:Person)-[:SUSPECTED_OF]->(:Incident {incident_id:'INC-007'}) RETURN p.name",
...
ITEMS["Q-085"] = (
    "MATCH (p:Person)-[:SUSPECTED_OF]->(:Incident {incident_id:'INC-006'}) RETURN p.name",
    [("Return a single (main) suspect", "MATCH (p:Person)-[:SUSPECTED_OF]->(:Incident {incident_id:'INC-006'}) RETURN p.name ORDER BY p.name LIMIT 1"),
     ("Return all suspects", "MATCH (p:Person)-[:SUSPECTED_OF]->(:Incident {incident_id:'INC-006'}) RETURN p.name")],
)
ITEMS["Q-086"] = (
...
ITEMS["Q-089"] = (
    "MATCH (p:Person)-[:SUSPECTED_OF]->(:Incident {incident_id:'INC-006'}), (p)-[:OWNS {active:true}]->(v:Vehicle) RETURN v.vehicle_id",
    [("Return the count of vehicles", "MATCH (p:Person)-[:SUSPECTED_OF]->(:Incident {incident_id:'INC-006'}), (p)-[:OWNS {active:true}]->(v:Vehicle) RETURN count(v) AS n"),
     ("Return the list of vehicles", "MATCH (p:Person)-[:SUSPECTED_OF]->(:Incident {incident_id:'INC-006'}), (p)-[:OWNS {active:true}]->(v:Vehicle) RETURN v.vehicle_id")],
)
ITEMS["Q-090"] = (
...
ITEMS["Q-095"] = (
    "MATCH (p:Person)-[:WITNESSED]->(:Incident {incident_id:'INC-006'}) RETURN p.name",
    [("Return the number of witnesses", "MATCH (p:Person)-[:WITNESSED]->(:Incident {incident_id:'INC-006'}) RETURN count(p) AS n"),
     ("Return the witnesses' names", "MATCH (p:Person)-[:WITNESSED]->(:Incident {incident_id:'INC-006'}) RETURN p.name")],
)
ITEMS["Q-096"] = (
    "MATCH (:Case {case_id:'CASE-001'})-[:CONTAINS]->(i:Incident) RETURN i.incident_id",
...
ITEMS["Q-101"] = lives("James Whitfield")
ITEMS["Q-102"] = lives("Lina Petrova")
ITEMS["Q-103"] = owns("Ricky Santos")
ITEMS["Q-104"] = owner_of("Holden", "Commodore")
ITEMS["Q-105"] = owner_of("Ford", "Ranger")
ITEMS["Q-106"] = uses("Lina Petrova")
ITEMS["Q-107"] = user_of("0412 345 678")
ITEMS["Q-108"] = user_of("0478 444 555")
ITEMS["Q-109"] = assoc("Tyler Birch")
ITEMS["Q-110"] = assoc("Amir Hassan")
ITEMS["Q-111"] = (
...
ITEMS["Q-112"] = lives("Amir Hassan")
ITEMS["Q-113"] = lives("Tyler Birch")
ITEMS["Q-114"] = lives("Connor Doyle")
ITEMS["Q-115"] = lives("Sarah Nguyen")
ITEMS["Q-116"] = owns("Nina Orlov")
ITEMS["Q-117"] = owner_of("Nissan", "Navara")
ITEMS["Q-118"] = owner_of("BMW", "3 Series")
ITEMS["Q-119"] = uses("Michael Nguyen")
ITEMS["Q-120"] = (
