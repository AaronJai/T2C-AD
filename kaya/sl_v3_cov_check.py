"""Step 7.4 acceptance criterion 4 — quick intrinsic check of `sl_adapter_v3`.

NOT a pipeline module: a Kaya verification harness. Loads the refreshed adapter
(checkpoints/{model_key}/sl_adapter_v3) on its base, regenerates diverse-beam completions for a
~25-item stratified sample of the v3 benchmark, and reports the sample Cov@5 (gold pattern
present in the top-5 beams) — the fast sanity read before the full 7.5 gate runs the same
measure over all 120 items with a live-Neo4j-backed E2E run. GPU only, no Neo4j needed (schema
linking alone). Reuses experiment/probe_utils.py's generate_beams/covered/gold_patterns (2.2/2.3)
so the measure is identical to the one 7.5 gates on.
"""
...
def main() -> None:
    ...
    print(f"Sampled {len(items)}/{args.sample_size} v3 items; decoding={decoding}")
    ...
    print(f"\nSample Cov@5 (sl_adapter_v3, n={len(items)}): {cov_at_5:.3f}")
