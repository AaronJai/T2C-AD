# 0.1 — Shared Types & Contracts

**Module:** `pipeline/types.py`
**Depends on:** — (foundational; no internal imports)
**Consumed by:** every subsequent step. This is the single source of truth for all
...
Define every dataclass and `Literal` that more than one pipeline stage exchanges, with
exact field names and types. No behaviour beyond two convenience methods and one private
serialiser. Once this file exists, no other step may redefine a type that lives here —
they import from `pipeline.types`.

---

## What this step owns vs. what lives elsewhere

**Owns (define here):** the type literals; `SchemaElement`, `SchemaRepr`;
`SchemaCandidate`, `CandidateMapping`; `EntityCandidate`, `EntityLookupResult`;
`AmbiguityResult`; `SchemaMapping`; `ValidationResult`; `ExecutionResult`;
`BenchmarkItem`; `EvaluationResult`; `PipelineState`; and the private helper
`_build_cypher_pattern`.

**Does NOT own (import from their step; never redefine):**

| Type | Lives in | Step |
|------|----------|------|
| `GenerationConfig`, `Completion`, `BaseLLM` | `pipeline/llm/base.py` | 0.2 |
| `build_pole_schema_repr()` (the *builder*; the `SchemaRepr` *dataclass* is here) | `pipeline/schema.py` | 0.3 |
| `CachedNode`, `EntityCache`, `ENTITY_REGISTRY` | `pipeline/entity_lookup/` | 3.2 |
| `MetricBundle`, `compute_metrics()` | `pipeline/evaluation/metrics.py` | 4.2 |
| `PipelineComponents` | `pipeline/components.py` | 5.1 |

> Note: `SchemaRepr`/`SchemaElement` are defined here (not in `schema.py`) because
> `PipelineState` and the mapping types reference them; defining them in `0.3` would make
> `0.1` depend on `0.3` and create a cycle. `0.3` only *builds* a `SchemaRepr`.

Also create an empty `pipeline/__init__.py` so `pipeline` is a package.

---

## The module — implement exactly
...
class PipelineState:
    ...
    # Attempt history (amended 2026-06-10). evaluation_result above holds only the FINAL
    # attempt; the metrics (4.2) need per-attempt data (Pass@1, repair rates), so:
    ...
    # Semantic-mismatch carry-forward (amended 2026-06-10). Accumulates every SchemaMapping
    # committed in this run. PERSISTS across BOTH disambiguation retries AND Schema Linker
    # outer-loop retries: the linker is deterministic (do_sample=False), so an outer retry
    # reproduces the same candidate set — persisting the tried list is what makes an outer
    # retry able to commit a *different* interpretation instead of repeating the failed one.
    # (Supersedes the original "cleared on outer reset" rule; see 5.2 retry-feedback design.)
...
## Edge cases & flagged decisions

- **`_build_cypher_pattern` branching:** as documented above, multi-branch patterns are
  comma-joined fragments, not a single connected path. Open question for the team: if QG
  quality on branching questions is poor, either (a) enrich this serialiser with variable
  reuse/chaining, or (b) make `top1_mapping()` skip `cypher_syntax` and let the QG work
  from `committed` alone. Decision deferred until 3.6 QG evaluation; record in
  `decisions-log.md` if changed.
- **`properties_score is None`:** legitimate (property-free query), not a failure. Routing
  treats `None` as "no property check needed" → does not route to schema linker.
- **`detected_types` may be multi-valued** even though benchmark ground truth is
  single-valued (compounds collapsed to one dominant type). Detection F1 (4.2) compares on
  the binary `is_ambiguous`, not on type-set equality, so this asymmetry is fine.
- **`MAX_*_RETRIES` are `init=False`:** they are constants, not constructor args. Do not
  pass them when building `PipelineState`.

---

## Acceptance criteria

Unit-testable now (no GPU/DB):

1. `from pipeline.types import *` imports cleanly; the module type-checks (no runtime
   errors constructing any dataclass with valid args).
2. **Benchmark contract test:** every row of `data/benchmark-updated.json` loads via
   `BenchmarkItem(**row)` for all 125 rows without error, and every resulting
   `ambiguity_type` is in `{"schema","entity","intent","temporal", None}`.
3. **`top1_mapping` contract test:** build a `CandidateMapping` with two mentions (each a
   list of ≥2 `SchemaCandidate` ordered by score), call `top1_mapping()`, assert the
   result is a `SchemaMapping` with `resolution_mode == "automated"` and `committed`
   holding exactly the rank-1 element per mention.
4. **`_build_cypher_pattern` contract test:** for a committed dict containing a
   `Person`→`SUSPECTED_OF`→`Incident` relationship plus a `crime_type` property on
   `Incident`, the output contains `[:SUSPECTED_OF]` and `{crime_type}` and is a single
   comma-joined string.

---

## Handoff (what the next steps rely on)

After this step, every later module imports its contracts from `pipeline.types`:
the type literals, all stage I/O dataclasses, `PipelineState`, and `_build_cypher_pattern`.
`0.2` adds the LLM interface (`BaseLLM`, `GenerationConfig`, `Completion`) independently;
`0.3` imports `SchemaRepr` from here to build the POLE schema. No later step redefines any
type listed in this file.
