...
Deferred (GPU + adapter): `SchemaLinker.link` over the benchmark; the **reported** intrinsic
Cov@5/EM@1 is computed on the **POLE benchmark** (gold pattern = `extract_schema_pattern` of the
benchmark's `cypher_default`/interpretations) — this is where 2.1's deferred Cov@5 target (≥85%)
is checked. The held-out text2cypher split is used only for the optional informal spot-check noted
in 2.1, not reported here.
